...
class Apps(http.Controller):
    ...
    # @http.route('/sd_apps/attendance/', type='http', auth="public", methods=['GET'], website=True)
    def sd_apps_http(self, **kwargs):
        ...
        if search:
            ...
            apps = http.request.env['sd_apps.settings'].sudo().search(['|',
                                                                       ('name', 'like', list[0]),
                                                                       ('link', 'like', list[0]),
            ...
            apps = http.request.env['sd_apps.settings'].sudo().search([], order='priority,name asc')
            return self.render_http(apps,time_1, today)
